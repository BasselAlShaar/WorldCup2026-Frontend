...
function parseLocalDate(dateStr) {
  ...
  const d = new Date(`${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}T${timePart ?? "00:00"}`);
...
function BracketMatchCard({ match }) {
  ...
  return (
    <div className={`rounded-lg border transition-all overflow-hidden h-full ${isTBD ? "border-white/5 bg-white/2 opacity-40" : isFinished ? "border-white/15 bg-white/5" : "border-gold/20 bg-gold/5"}`}>
      {[{ team: match.homeTeam, score: match.homeScore }, { team: match.awayTeam, score: match.awayScore }].map((side, i) => (
        <div key={i} className={`flex items-center gap-2 px-3 py-2 ${i === 0 ? "border-b border-white/8" : ""}`}>
          <span className="font-body text-xs font-medium text-white/80 flex-1 truncate">{side.team || "TBD"}</span>
          {!isUpcoming && side.score !== null && side.score !== undefined && (
            <span className={`font-mono text-sm font-bold ${isFinished ? "text-white" : "text-gold"}`}>{side.score}</span>
          )}
        </div>
      ))}
    </div>
...
function AbsoluteColumn({ label, count, totalHeight, width, children }) {
  return (
    <div className="flex-shrink-0" style={{ width }}>
      <p className="font-display text-xs text-gold/70 tracking-widest text-center" style={{ height: `${LABEL_HEIGHT}px`, lineHeight: `${LABEL_HEIGHT}px` }}>{label}</p>
      <div className="relative" style={{ height: `${totalHeight}px` }}>
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="absolute left-0 right-0" style={{ top: `${cardTopY(i, count, totalHeight)}px`, height: `${CARD_HEIGHT}px` }}>
            {children(i)}
          </div>
        ))}
      </div>
...
function BracketConnectors({ leftCount, rightCount, totalHeight }) {
  ...
  for (let p = 0; p < pairs; p++) {
    ...
    paths.push(<path key={`t-${p}`} d={`M 0 ${y1} H ${xElbow} V ${yMerge}`} fill="none" stroke="#D4AF37" strokeWidth="1" strokeOpacity="0.45" />);
    paths.push(<path key={`b-${p}`} d={`M 0 ${y2} H ${xElbow} V ${yMerge}`} fill="none" stroke="#D4AF37" strokeWidth="1" strokeOpacity="0.45" />);
    paths.push(<path key={`m-${p}`} d={`M ${xElbow} ${yMerge} V ${yRight} H ${CONNECTOR_WIDTH}`} fill="none" stroke="#D4AF37" strokeWidth="1" strokeOpacity="0.45" markerEnd="url(#arrowhead)" />);
  }
  return (
    <svg width={CONNECTOR_WIDTH} height={totalHeight} className="flex-shrink-0" style={{ display: "block", marginTop: `${LABEL_HEIGHT}px` }}>
      <defs>
...
function DesktopBracket({ rounds, totalHeight }) {
  return (
    <div className="overflow-x-auto pb-4" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(212,175,55,0.2) transparent" }}>
      <div className="flex items-start gap-0" style={{ width: "max-content", paddingBottom: "8px" }}>
        {rounds.map((round, ri) => (
          <Fragment key={round.round}>
            <AbsoluteColumn label={round.round} count={round.matches.length} totalHeight={totalHeight} width="180px">
              {(i) => <BracketMatchCard match={round.matches[i]} />}
            </AbsoluteColumn>
            {ri < rounds.length - 1 && (
              <BracketConnectors leftCount={round.matches.length} rightCount={rounds[ri + 1].matches.length} totalHeight={totalHeight} />
            )}
          </Fragment>
        ))}
        <div className="flex-shrink-0 flex items-center justify-center px-6 opacity-60" style={{ marginTop: `${LABEL_HEIGHT}px`, height: `${totalHeight}px` }}>
...
async function fetchBracket() {
  const res = await fetch("https://worldcup26.ir/get/games", { cache: "no-store" }).then((r) => r.json());
  ...
  const matches = knockoutGames.map((g) => {
    ...
    const roundLabel = ROUND_LABELS[g.type] ?? g.type?.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()) ?? "Unknown";
    return {
      id: g._id,
      round: roundLabel,
  ...
  const hasRealTeams = matches.some((m) => m.homeTeam !== "TBD" && m.awayTeam !== "TBD");
  ...
  const extraRounds = Object.keys(grouped).filter((r) => !ORDERED_ROUNDS.includes(r));
  return {
    rounds: [...orderedRounds, ...extraRounds].map((r) => ({ round: r, matches: grouped[r] })),
    isEmpty: false,
  };
}

export default function BracketPage() {
  const fetcher = useCallback(() => fetchBracket(), []);
  const { data, loading } = usePolling(fetcher, 30000);

  const isEmpty = !data || data.isEmpty;
  const rounds = isEmpty ? EMPTY_ROUNDS : data.rounds;
  const maxMatchCount = Math.max(...rounds.map((r) => r.matches.length));
  const totalHeight = maxMatchCount * CARD_HEIGHT * 1.4;

  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      <div className="mb-10 animate-fade-in">
        <h1 className="font-display text-5xl md:text-6xl text-white">Knockout Bracket</h1>
        <p className="text-white/40 font-body text-sm mt-1">Road to the World Cup 2026 Final</p>
        {isEmpty && !loading && (
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="md:hidden space-y-8">
            {rounds.map((round) => (
              <div key={round.round}>
                <h2 className="text-gold font-display text-sm tracking-widest uppercase mb-3">{round.round}</h2>
